...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			// music.loadStream(Paths.music('techdevpresent'));
			// FlxG.sound.list.add(music);
			// music.play();

			if(FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music('techdevpresent'), 0);
			}
		}
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 2:
					#if PSYCH_WATERMARKS
					createCoolText(['Tech Engine by'], 15);
					#else
					createCoolText(['techdeveloper', 'bho']);
					#end
				// credTextShit.visible = true;
				case 3:
					addMoreText('techdeveloper', 15);
					addMoreText('superpowers04', 15);
					addMoreText('bho', 15);
